...
class ActivationSAEAnalyzer:
    ...
    def generate_activations_for_patterns(self, target_state: str = "depressed") -> Dict[str, torch.Tensor]:
        ...
        activations = self.activation_capturer.capture_activations(
            ...
            position="last_10",  # Capture last 10 token positions
            use_cache=True
        )
    ...
    def run_sae_analysis(self, activations: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        acts = acts.to(self.sae.W_enc.dtype)  # Match SAE dtype
        
        results = {}
        
        with torch.no_grad():
            # For each pattern (first dimension), analyze each token position separately
            for pattern_idx in range(acts.shape[0]):
                ...
                # Get activations for this pattern - shape: [seq_len, hidden_size] (last 10 tokens)
                pattern_acts = acts[pattern_idx]
                seq_len = pattern_acts.shape[0]
                ...
                # Analyze each token position separately
                token_analysis = []
                for token_pos in range(seq_len):
                    # Get feature activations for this specific token position
                    token_feature_acts = feature_acts[token_pos]  # [n_features]
                    
                    # Get top 30 features for this token position
                    top_values, top_indices = torch.topk(token_feature_acts, k=30)
                    
                    token_analysis.append({
                        'position': token_pos,
                        'relative_position': token_pos - seq_len,  # -10, -9, -8, ..., -1 (last 10 tokens)
                        'l0_norm': (token_feature_acts > 0).sum().item(),
                        'top_features': [(idx.item(), val.item()) for idx, val in zip(top_indices, top_values)],
                        'max_activation': top_values[0].item() if len(top_values) > 0 else 0.0
                    })
                ...
                results[pattern_name] = {
                    ...
                    'feature_activations': feature_acts.cpu(),  # Keep for compatibility
                    'reconstruction': sae_out.cpu()
                }
                
                print(f"   {pattern_name}:")
                print(f"     Sequence length: {seq_len} tokens")
                print(f"     Avg L0 norm: {l0_norm.mean().item():.2f}")
                print(f"     Reconstruction MSE: {reconstruction_mse.item():.6f}")
                print(f"     Max token activation: {max(t['max_activation'] for t in token_analysis):.4f}")
    ...
    def fetch_feature_descriptions(self, feature_indices: List[int]) -> Dict[int, Dict[str, Any]]:
        """Fetch descriptions for a list of feature indices with incremental saving."""
        print(f"\n📖 Fetching descriptions for {len(feature_indices)} features...")
        
        # Setup in
        #incremental save file
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        incremental_save_file = self.output_dir / f"feature_descriptions_incremental_{timestamp}.json"
        
        # Try to find and load the most recent incremental save
        descriptions = {}
        existing_saves = list(self.output_dir.glob("feature_descriptions_incremental_*.json"))
        if existing_saves:
            # Get the most recent save file
            most_recent_save = max(existing_saves, key=lambda x: x.stat().st_mtime)
            try:
                with open(most_recent_save, 'r') as f:
                    descriptions = {int(k): v for k, v in json.load(f).items()}
                print(f"   📂 Loaded {len(descriptions)} existing descriptions from {most_recent_save.name}")
                # Use the existing file for continued saving
                incremental_save_file = most_recent_save
            except Exception as e:
                print(f"   ⚠️ Could not load incremental save: {e}")
                descriptions = {}
        else:
            print(f"   📝 Starting fresh - will save to {incremental_save_file.name}")
        ...
        remaining_features = [f for f in feature_indices if f not in descriptions]
        print(f"   📋 Need to fetch {len(remaining_features)} new features")
        
        for i, feature_idx in enumerate(remaining_features):
            print(f"   Fetching {i+1}/{len(remaining_features)}: Feature {feature_idx}")
            ...
            # Incremental save every 50 features
            if (i + 1) % 50 == 0 or (i + 1) == len(remaining_features):
                try:
                    with open(incremental_save_file, 'w') as f:
                        json.dump(descriptions, f, indent=2)
                    print(f"   💾 Saved progress: {len(descriptions)} features to {incremental_save_file.name}")
                except Exception as e:
                    print(f"   ⚠️ Could not save incremental progress: {e}")
    ...
    def create_analysis_dataset(self, sae_results: Dict[str, Any]) -> Dict[str, Any]:
        """Create structured dataset with token-based analysis results and descriptions."""
        print(f"\n📊 Creating analysis dataset...")
        
        # Collect all unique features across all tokens and patterns
        all_feature_indices = set()
        for pattern_name, results in sae_results.items():
            for token_data in results['token_analysis']:
                for feature_idx, _ in token_data['top_features'][:10]:  # Top 10 per token
                    all_feature_indices.add(feature_idx)
        
        print(f"   Found {len(all_feature_indices)} unique features across all tokens and patterns")
        ...
        dataset = {
            'analysis_metadata': {
                ...
                'analysis_type': 'per_token_last_10'
            },
            'patterns': {}
        }
        ...
        for pattern_name, results in sae_results.items():
            # Create a summary of top features across all tokens for this pattern
            pattern_feature_summary = {}
            for token_data in results['token_analysis']:
                for feature_idx, activation in token_data['top_features'][:5]:  # Top 5 per token
                    if feature_idx not in pattern_feature_summary:
                        pattern_feature_summary[feature_idx] = 0.0
                    pattern_feature_summary[feature_idx] += activation
            
            # Get top features for pattern summary
            sorted_features = sorted(pattern_feature_summary.items(), key=lambda x: x[1], reverse=True)[:10]
            pattern_features = []
            
            for i, (feature_idx, total_activation) in enumerate(sorted_features):
                feature_info = {
                    'rank': i + 1,
            ...
            dataset['patterns'][pattern_name] = {
                ...
                'top_features': pattern_features  # Summary for compatibility
            }
    ...
    def _create_report(self, report_file: Path, dataset: Dict[str, Any]) -> None:
        """Create a markdown report of the per-token SAE analysis results."""
        with open(report_file, 'w') as f:
            f.write("# Per-Token SAE Analysis Report\\n\\n")
            ...
            f.write(f"**Analysis Type:** Last 10 tokens per pattern\\n\\n")
            f.write(f"**Top Features per Token:** 30\\n\\n")
            f.write(f"**SAE:** {metadata['sae_config']['release']}/{metadata['sae_config']['sae_id']}\\n\\n")
            
            # Pattern results with per-token analysis
            f.write("## Per-Token Analysis Results\\n\\n")
            
            for pattern_name, pattern_data in dataset['patterns'].items():
                f.write(f"### {pattern_name}\\n\\n")
                f.write(f"**Overall Metrics:**\\n")
                f.write(f"- Sequence Length: {pattern_data['sequence_length']} tokens\\n")
                f.write(f"- Avg L0 Sparsity: {pattern_data['avg_l0_norm']:.2f}\\n")
                f.write(f"- Reconstruction MSE: {pattern_data['reconstruction_mse']:.6f}\\n\\n")
                f.write(f"**Text Example:** {pattern_data['pattern_text'][:200]}...\\n\\n")
                
                # Token-by-token analysis table
                f.write("#### Token-by-Token Feature Analysis\\n\\n")
                
                # Create a table for each token position
                token_analysis = pattern_data['token_analysis']
                for token_data in token_analysis:
                    rel_pos = token_data['relative_position']
                    f.write(f"**Token Position {rel_pos} (L0: {token_data['l0_norm']}, Max: {token_data['max_activation']:.4f})**\\n\\n")
                    
                    # Table header
                    f.write("| Rank | Feature | Activation | Description | Activates On |\\n")
                    f.write("|------|---------|------------|-------------|--------------|\\n")
                    
                    # Top 30 features for this token
                    for rank, (feature_idx, activation) in enumerate(token_data['top_features'], 1):
                        # Get feature description if available
                        feature_info = None
                        for pattern_features in dataset['patterns'].values():
                            if 'top_features' in pattern_features:
                                for feature in pattern_features['top_features']:
                                    if feature['feature_index'] == feature_idx:
                        ...
                        if feature_info:
                            desc = feature_info.get('description', 'No description')[:50]
                            pos_tokens = ', '.join(feature_info.get('pos_tokens', [])[:3])
                        else:
                            desc = 'No description available'
                            pos_tokens = 'Unknown'
                        
                        f.write(f"| {rank} | {feature_idx} | {activation:.4f} | {desc}{'...' if len(desc) == 50 else ''} | {pos_tokens} |\\n")
